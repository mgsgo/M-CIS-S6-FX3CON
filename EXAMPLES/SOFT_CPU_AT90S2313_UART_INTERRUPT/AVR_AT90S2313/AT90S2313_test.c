...
//UART interrupt
unsigned char uart_int_value;
SIGNAL(SIG_UART_RECV) // USART 1 Rx Complete interrupt function
{
	unsigned char rx_byte;
	rx_byte = UDR; // copy from host rx byte to communication byte

	uart_int_value = rx_byte;
}

/* initialize UART */
void InitUART( unsigned char baudrate )
{
    UCR = ( (1<<RXEN) | (1<<TXEN) | (1<<RXCIE)); /* enable UART receiver and transmitter */
...
int main(void){
    ...
    TransmitLength("MGSG \n", 6);
    TransmitString("AT90S2313 soft core test int\0");
